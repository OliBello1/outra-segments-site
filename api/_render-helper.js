...
function buildCommercialsHtml(record) {
  ...
  function buildBonsoirSlider(cfg, accent) {
    ...
    const bands = Array.isArray(s.bands) && s.bands.length ? s.bands : [
      { from: 0,     to: 9999,  oneOff: 1.25, m3: 1.05, m6: 1.00 },
    ...
    return ''
      ...
      // LEFT column: live readouts + sliders
      + '<div class="bns-left">'
  ...
  function buildBonsoirRoiScript() {
    return '\n<script>(function(){\n'
      ...
      + 'function bnsTween(el,to,fmt){\n'
      + '  if(!el) return; var from=(el.__bnsVal==null?to:el.__bnsVal);\n'
      + '  if(el.__bnsRaf) cancelAnimationFrame(el.__bnsRaf);\n'
      + '  if(from===to){ el.__bnsVal=to; el.textContent=fmt(to); return; }\n'
      + '  var dur=420, t0=(window.performance&&performance.now?performance.now():Date.now());\n'
      + '  function ease(p){return 1-Math.pow(1-p,3);}\n'
      + '  function step(now){ var p=Math.min(1,(now-t0)/dur); var v=from+(to-from)*ease(p);\n'
  ...
  function buildLoafCompactCss(styleV2) {
    return '\n<style>\n'
      ...
      + '.prop-commercials:has(.loaf-cp){min-height:100vh;box-sizing:border-box;display:flex;flex-direction:column;justify-content:center;padding-top:28px;padding-bottom:28px;}\n'
      + '.loaf-cp .prop-commercials-header{margin-bottom:18px;text-align:center;}\n'
      + '.loaf-cp .prop-commercials-title{font-size:28px;margin-bottom:6px;}\n'
      + '.loaf-cp .prop-commercials-sub{max-width:760px;margin-left:auto;margin-right:auto;font-size:15px;}\n'
      + '.loaf-cp .prop-pricing-grid{gap:16px;align-items:stretch;}\n'
      ...
      + '.loaf-cp .prop-pricing-card{padding:12px 18px;}\n'
      + '.loaf-cp .prop-pricing-card-name{margin-bottom:4px;font-size:16px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}\n'
      + '.loaf-cp .prop-refresh-pill{margin:0 0 4px;}\n'
      + '.loaf-cp .prop-slider-wrap{margin:4px 0 5px;}\n'
      + '.loaf-cp .prop-slider-sites-num{font-size:24px;line-height:1.1;}\n'
      + '.loaf-cp .prop-slider-sites-label{font-size:14px;}\n'
      + '.loaf-cp .prop-slider-ticks{font-size:12px;}\n'
      + '.loaf-cp .prop-price-display{margin:4px 0 2px;}\n'
      + '.loaf-cp .prop-price-num{font-size:23px;}\n'
      + '.loaf-cp .prop-price-meta{margin-bottom:4px;font-size:14px;min-height:0;}\n'
      + '.loaf-cp .prop-tier-table{margin-top:4px;}\n'
      // Revealed volume-pricing breakdown (opt-in via slider.show_tiers).
      + '.loaf-cp .loaf-tier-heading{margin-top:6px;font-size:12px;font-weight:700;letter-spacing:0.3px;text-transform:uppercase;color:rgba(255,255,255,0.45);}\n'
      + '.loaf-cp .prop-tier-table.loaf-tier-show{display:block;border-top:1px solid rgba(255,255,255,0.12);padding-top:5px;margin-top:4px;}\n'
      ...
      + '.loaf-cp .prop-tier-row{padding:1px 0;font-size:14px;}\n'
      + '.loaf-cp .unlimited-price{font-size:32px;line-height:1.1;margin-bottom:4px;}\n'
      + '.loaf-cp .unlimited-period{margin-bottom:12px;font-size:16px;}\n'
      + '.loaf-cp .unlimited-features{margin-top:10px;}\n'
      + '.loaf-cp .unlimited-features li{margin-bottom:4px;font-size:16px;}\n'
      + '.loaf-cp .prop-commercials-footnote{margin-top:14px;font-size:13px;}\n'
  ...
  function buildThreeColCss() {
    return '\n<style>\n'
      ...
      /* tiles are 420x200. Width is a third of the card's content box minus the
         two gaps, so a three-channel column fits on one row and never wraps —
         and a single-tile column (Zap Post, Reapit) renders at exactly the same
         size, so every media strip is one row of identical tiles on a shared
         line across all three cards */
      + '.tc-cp-media-tiles{display:flex;flex-wrap:nowrap;gap:10px;align-items:center;justify-content:center;width:100%;}\n'
      + '.tc-cp-media-tiles img{width:calc((100% - 20px) / 3);max-width:118px;height:auto;display:block;border-radius:12px;}\n'
